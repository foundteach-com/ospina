...
export default function EditClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="p-8 text-gray-600 text-center">Cargando datos del cliente...</div>;
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-8 space-y-6 shadow-sm">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Nombre Completo / Razón Social *</label>
          <input
            type="text"
            name="name"
            required
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Identificación (NIT / Cédula) *</label>
          <input
            type="text"
            name="taxId"
            required
        ...
        <div className="pt-4 flex justify-end gap-4">
          ...
          <button
            type="submit"
            disabled={saving}
            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
