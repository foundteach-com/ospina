...
export default function CashFlowPage() {
  ...
  const exportPDF = () => {
     ...
     // Summary
     doc.text(`Ingresos: $${summary.totalIncome.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`, 14, 40);
     doc.text(`Gastos: $${summary.totalExpense.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`, 80, 40);
     doc.text(`Balance: $${summary.balance.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`, 140, 40);
     ...
     const tableRows = records.map(record => [
       ...
       `$${record.amount.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
     ]);
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
          ...
          <p className="text-3xl font-bold text-green-600">${summary.totalIncome.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p>
        </div>
        <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
          <h3 className="text-gray-500 font-medium mb-1">Total Gastos</h3>
          <p className="text-3xl font-bold text-red-600">${summary.totalExpense.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p>
        </div>
        <div className={`p-6 rounded-2xl border shadow-sm ${summary.balance >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          ...
          <p className={`text-3xl font-bold ${summary.balance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ${summary.balance.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
          </p>
        </div>
      </div>
                ...
                records.map((record) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm font-bold text-right">
                      {record.type === 'INCOME' ? '+' : '-'}${record.amount.toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                    </td>
